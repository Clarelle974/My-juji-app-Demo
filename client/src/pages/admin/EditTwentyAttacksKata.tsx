...
import { putTwentyAttacksKata } from "../../services/eadRequests";

export default function EditTwentyAttacksKata() {
  ...
  const handleEditKata = async (editedKata: KataData) => {
    ...
    const response = await putTwentyAttacksKata(id, editedKata);
    if (response?.errors) {
      Swal.fire({
      ...
      navigate(`/twenty-attacks#${kata.id}`);
    } else {
      Swal.fire({
        title: "Erreur",
        icon: "error",
